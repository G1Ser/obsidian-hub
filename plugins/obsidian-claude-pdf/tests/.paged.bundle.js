...
`;
    ...
    const isOverflow = (content) => {
      return content.scrollHeight > content.clientHeight + 1;
    };
    const appendNormal = (block, contentRef2) => {
      const cloned = block.cloneNode(true);
      contentRef2.current.appendChild(cloned);
      if (!isOverflow(contentRef2.current)) return;
      cloned.remove();
      contentRef2.current = createPage();
      contentRef2.current.appendChild(block.cloneNode(true));
    };
    const appendImage = (block, contentRef2) => {
      const img = block.cloneNode(true);
      img.style.maxWidth = "100%";
      img.style.height = "auto";
      contentRef2.current.appendChild(img);
      if (!isOverflow(contentRef2.current)) return;
      img.remove();
      contentRef2.current = createPage();
      contentRef2.current.appendChild(img);
      if (isOverflow(contentRef2.current)) {
        img.style.maxHeight = `${contentRef2.current.clientHeight}px`;
        img.style.width = "auto";
        img.style.objectFit = "contain";
      }
    };
    const appendTable = (table, contentRef2) => {
      const rows = Array.from(table.querySelectorAll("tbody > tr"));
      if (!rows.length) {
        appendNormal(table, contentRef2);
        return;
      }
      const createEmptyTable = () => {
        ...
        next.appendChild(tbody2);
        return next;
      };
      let currentTable = createEmptyTable();
      let tbody = currentTable.querySelector("tbody");
      contentRef2.current.appendChild(currentTable);
      for (const row of rows) {
        const clonedRow = row.cloneNode(true);
        tbody.appendChild(clonedRow);
        if (!isOverflow(contentRef2.current)) continue;
        clonedRow.remove();
